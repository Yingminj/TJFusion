...
class BridgeResultPublisher:
    def __init__(
        ...
        *,
        ...
        siglip_sync_with_pose: bool = False,
        siglip_pose_wait_timeout_sec: float = 0.0,
    ) -> None:
        ...
        self._siglip_sync_with_pose = bool(siglip_sync_with_pose)
        self._siglip_pending_samples: deque[dict[str, Any]] = deque(maxlen=self._siglip_vote_window)
        self._siglip_pose_wait_timeout_sec = max(0.0, float(siglip_pose_wait_timeout_sec))
        self._siglip_buffer_start_monotonic: float | None = None
    ...
    def _append_pending_siglip_sample(self, siglip_result: dict[str, Any]) -> None:
        category = siglip_result.get("best_category")
        if not bool(siglip_result.get("ok", False)):
            return
        ...
        if not normalized:
            return
        self._siglip_pending_samples.append(
            {
                "best_category": normalized,
                "best_similarity": siglip_result.get("best_similarity"),
            }
        )
        if self._siglip_buffer_start_monotonic is None:
            self._siglip_buffer_start_monotonic = time.monotonic()

    def _select_pending_vote(self) -> tuple[Any, Any]:
        if not self._siglip_pending_samples:
            return None, None

        counts = Counter(sample["best_category"] for sample in self._siglip_pending_samples)
        max_count = max(counts.values(), default=0)
        winners = {name for name, freq in counts.items() if freq == max_count}
        for sample in reversed(self._siglip_pending_samples):
            if sample["best_category"] in winners:
                return sample["best_category"], sample.get("best_similarity")
        latest = self._siglip_pending_samples[-1]
        return latest["best_category"], latest.get("best_similarity")
    ...
    def publish(self, result: dict[str, Any]) -> None:
        ...
        pose_ready = len(tf_payload["transforms"]) > 0

        if self._siglip_sync_with_pose:
            self._append_pending_siglip_sample(siglip_result)
            timed_out = False
            if (
                not pose_ready
                and self._siglip_pose_wait_timeout_sec > 0.0
                and self._siglip_pending_samples
                and self._siglip_buffer_start_monotonic is not None
            ):
                waited = time.monotonic() - self._siglip_buffer_start_monotonic
                timed_out = waited >= self._siglip_pose_wait_timeout_sec
            if not pose_ready and not timed_out:
                print_status(
                    "PUB",
                    (
                        "buffering siglip results pending_pose "
                        f"buffer_size={len(self._siglip_pending_samples)} "
                        f"vote_k={self._siglip_vote_window}"
                    ),
                    color="yellow",
                )
                return
            best_category, best_similarity = self._select_pending_vote()
            siglip_ok = best_category is not None
        else:
        ...
        print_status(
            ...
            (
                ...
                f"sync_with_pose={self._siglip_sync_with_pose}"
            ),
            color="green",
        )
        if self._siglip_sync_with_pose:
            self._siglip_pending_samples.clear()
            self._siglip_buffer_start_monotonic = None
